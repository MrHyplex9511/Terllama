...
inline void ternary_linear_dispatch(const LayerData& layer,
                                     const float* input, float* output) {
    if (layer.has_raw_weights) {
        // float*float matmul for raw FP32 layers (e.g., lm_head)
...
// RMS NORMALIZATION
// ═══════════════════════════════════════════════════════════════════════════
inline void rms_norm(float* x, const float* weight, int n, float eps) {
    float ss = 0.0f;
...
inline RoPECache build_rope_cache(int max_seq_len, int head_dim, float theta) {
    RoPECache c;
...
inline void apply_rope(float* q, float* k, int seq_pos, int n_heads, int n_kv_heads,
...
// ATTENTION
// ═══════════════════════════════════════════════════════════════════════════
inline void attention(float* x, int seq_pos, const ModelConfig& cfg,
...
inline void mlp_forward(float* x, const LayerData& gate_proj, const LayerData& up_proj,
...
// TRANSFORMER BLOCK
// ═══════════════════════════════════════════════════════════════════════════
inline void transformer_block(float* x, int seq_pos, int layer_idx,
...
// FULL MODEL FORWARD
// ═══════════════════════════════════════════════════════════════════════════
inline float* model_forward(int token, int seq_pos,
                             const ModelConfig& cfg,
    ...
    if (x.size() != (size_t)cfg.hidden_size) x.resize(cfg.hidden_size);
    ...
    ternary_linear_dispatch(layers[idx_lmhead], x.data(), logits.data());
...
inline int sample_multinomial(const float* logits, int n, float temp,
    ...
    for (int i = 0; i < n; i++) {
        float val = logits[i];
        // Repetition penalty: scale logits of recent tokens
...
inline std::pair<std::vector<int>, double> generate(
    ...
    for (int i = 0; i < max_tokens; i++) {
        ...
        // Recent tokens for repetition penalty (last 8)
